...
-- Comprehensive query logic for all sectors and funds
;WITH BaseData AS (
    SELECT
        pos.Fund,
        pos.Sector,
...
Positions_CTE AS (
    SELECT 
        ...
        SUM(pos.Marketvalue) / (SUM(SUM(pos.Marketvalue)) OVER()) AS 'Allocation'
    FROM fund.positions pos
...
ORDER BY 
    ...
    CASE 
        ...
        WHEN pos.Rating = 'AAA' THEN 6
        WHEN pos.Rating = 'AA/A' THEN 7
        WHEN pos.Rating = '<=BB' THEN 8
        WHEN pos.Rating = '<=BBB' THEN 9
        ELSE 10 
    END ASC;
